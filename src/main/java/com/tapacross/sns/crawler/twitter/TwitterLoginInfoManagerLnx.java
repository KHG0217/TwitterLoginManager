...
public class TwitterLoginInfoManagerLnx {
	...
	private final Logger logger = LoggerFactory.getLogger(TwitterLoginInfoManagerLnx.class);
	...
	public static void main(String[] args) { 		
		TwitterLoginInfoManagerLnx manger = new TwitterLoginInfoManagerLnx();
		String proxyPath ="target/classes/data/haiip.txt";		
		manger.initSpringBeans();	
		try {
			while(true) {					
			String date = manger.currentDate();
			manger.logger.info(TWITTER_LOGIN_INFO_MANAGER + ", ----------------UPDATE START---------------------------------");
			manger.logger.info(TWITTER_LOGIN_INFO_MANAGER + ", ----------------CURRENT DATE: " + date + "-------------");	
					manger.updateTwitterAuthData(null, 0);	
					manger.logger.info(TWITTER_LOGIN_INFO_MANAGER + ", ----------------UPDATE END---------------------------------");
					manger.logger.info(TWITTER_LOGIN_INFO_MANAGER + ", updateCount: " + updateCount);
					manger.logger.info(TWITTER_LOGIN_INFO_MANAGER + ", updateFailCount: " + updateFailCount);
					for(String failId : updatefailList) {
						manger.logger.info(TWITTER_LOGIN_INFO_MANAGER + ", " + failId);
					}
				}
...
//		        options.addArguments("--proxy-server=http://" + proxyIp + ":" + port);
				...
				String WEB_DRIVER_PATH = "chromedriver"; // 드라이버 경로
...
//					System.out.println("Change port : " + port);
	...
	public void updateTwitterAuthData(String proxyIp, int port) {	
		try {
			...
			statusFList = service.select401TwitterId();
			
			if(statusFList.size() == 0) {
				logger.info(TWITTER_LOGIN_INFO_MANAGER + ", Status Z email size 0");
				logger.info(TWITTER_LOGIN_INFO_MANAGER + ", Thread Sleep 60 sec");
...
//				String pwTextXpath ="/html/body/div/div/div/div[1]/div/div/div/div/div/div/div[2]/div[2]/div/div/div[2]/div[2]/div[1]/div/div/div[1]/div/h1/span/span";
				...
				logger.info(TWITTER_LOGIN_INFO_MANAGER + ", pwText : " + pwText);
...
//					}
	...
	public void driverActions_Keys(WebDriver driver, Keys key, int count) {
		...
		while(count > num) {
			...
			logger.info(TWITTER_LOGIN_INFO_MANAGER + ", " + key.name());
			ThreadUtil.sleep(500);
			num ++;
		}			
	}
